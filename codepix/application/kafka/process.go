...
func NewKafkaProcessor(database *gorm.DB, producer *ckafka.Producer, deliveryChan chan ckafka.Event) *Processor {
	return &Processor{
		Database:     database,
		Producer:     producer,
...
func (k *Processor) processMessage(msg *ckafka.Message) {
...
func (k *Processor) processTransaction(msg *ckafka.Message) error {
...
func (k *Processor) processTransactionConfirmation(msg *ckafka.Message) error {
	transaction := appmodel.NewTransaction()
	err := transaction.ParseJson(msg.Value)
	if err != nil {
		return err
...
func (k *Processor) confirmTransaction(transaction *appmodel.Transaction, useCase usecase.TransactionUseCase) error {
